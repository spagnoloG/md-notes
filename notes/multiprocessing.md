...
- **Manager / worker** - Manager: - Dynamically spawns worker on need - Workers should be independent of eachother - Worker: - Processes data passed by the Manager - Saves the result or returns it to the Manager
...
Default: - Most of the variables are shared - Threads share global variables - Variables that are in parallel blocks, are not shared - Variables of the forked programs that are called from parrallel sections, are local for every thread
...
Compiler function labels: - `__global__` ~ Function is ran on device, and the one calling it is the host - `__device__` ~ Function is ran on device, and can be called only from within device - `__host__` ~ Function is ran on the host ( default )
